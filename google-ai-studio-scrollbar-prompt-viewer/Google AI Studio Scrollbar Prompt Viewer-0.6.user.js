...
(function () {
  ...
  function updateLabels() {
    const scrollbar = document.querySelector(
      'ms-prompt-scrollbar[role="toolbar"]',
    );
    if (!scrollbar) {
    ...
    const existingLabels = new Map(
      Array.from(labelContainer.children).map((node) => [
        node.dataset.key,
        node,
      ]),
    );
  ...
  function setupKeyboardShortcuts() {
    document.addEventListener("keydown", (event) => {
      ...
      const buttons = Array.from(
        scrollbar.querySelectorAll("button[aria-label]"),
      );
      ...
      const currentIndex = buttons.findIndex((btn) =>
        btn.classList.contains("ms-button-active"),
      );
      ...
      if (event.key === "j") {
        ...
        // Go to PREVIOUS prompt (event.key === 'k')
