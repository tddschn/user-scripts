...
(function() {
    ...
    function addDownloadButton(playButton) {
        ...
        if (playButton.nextElementSibling && playButton.nextElementSibling.classList.contains('audio-download-button')) {
            return;
        }
        ...
        try {
            ...
            console.error('MOE Audio Downloader: Failed to parse audio source:', srcAttr, e);
            return;
        }
        ...
        if (!filename) {
            filename = relativeUrl.split('/').pop().replace(/\.mp3$/, '');
        }
        filename += '.mp3';
        ...
        downloadLink.className = 'btn btn-outline-secondary border-0 p-0 ms-1 lh-normal audio-download-button';
