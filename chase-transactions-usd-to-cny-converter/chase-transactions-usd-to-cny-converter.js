...
(function() {
                ...
                onload: function(response) {
                    if (response.status === 200) {
                        try {
                            ...
                            if (rate) {
                                console.log(`[Chase CNY Converter] Fetched latest ${BASE_CURRENCY}/${TARGET_CURRENCY} rate: ${rate}`);
                                resolve(rate);
                            } else {
                ...
                onload: function(response) {
                    if (response.status === 200) {
                        try {
                            ...
                            console.log(`[Chase CNY Converter] Fetched ${Object.keys(rates).length} historical rates`);
                            resolve(rates);
                        } catch (e) {
                            reject('Failed to parse historical rates response.');
                        }
                    } else {
                        reject(`Historical rates API request failed with status: ${response.status}`);
                    }
                },
                onerror: function(error) {
    ...
    function processTransactions() {
        ...
        rows.forEach(row => {
            ...
            const rate = transactionDate ? getRateForDate(transactionDate) : fallbackRate;

            if (!rate) {
                console.warn(`[Chase CNY Converter] No rate available for date: ${transactionDate}`);
                return;
            }

            // The description is the first <td>. The amount column is different for pending vs. posted transactions.
            const isPending = row.closest('.pending__container') || row.id.includes('PENDING');
    ...
    async function fetchRatesAndProcess() {
        ...
        if (uncachedDates.length > 0) {
            ...
            try {
                ...
                console.warn(`[Chase CNY Converter] Failed to fetch historical rates: ${error}`);
            }
        }
     ...
     */
    async function main() {
        console.log('[Chase CNY Converter] Script started (v2.0 - Historical Rates).');
        try {
