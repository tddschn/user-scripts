...
(function () {
  ...
  function createControlPanel() {
    ...
    document
      .getElementById("gm-start-download")
  ...
  async function startFullDownload() {
    ...
    const totalPages = parseInt(
      document.getElementById("page-sp2").innerText,
      10,
    );
    let startPage = parseInt(
      document.getElementById("gm-start-page-input").value,
      10,
    );
    let pagesToDownload = parseInt(
      document.getElementById("gm-page-count-input").value,
      10,
    );
    ...
    const startPageLink = document.querySelector(
      `#pic-ul .nav-item:nth-child(${startPage}) .nav-link`,
    );
    if (!startPageLink) {
    ...
    for (
      ...
      const iframe = document.querySelector(
        'iframe[src*="viewer.html?file=blob:"]',
      );
      if (!iframe) {
      ...
      if (pdfBlobUrl) {
        try {
          ...
          console.error(`Failed to fetch PDF for page ${currentPageNum}:`, e);
        }
      }
  ...
  async function mergeAndOfferDownload(pdfBuffers, startPage, endPage) {
    const finalLinksDiv = document.getElementById("gm-final-links");
    const documentTitle =
      document.querySelector("h1.navbar-brand").innerText.trim() || "document";
    const pageRange =
      startPage === endPage
        ? `page ${startPage}`
    ...
    for (const buffer of pdfBuffers) {
      try {
        ...
        console.error(
          "Could not process a page, it might be corrupted. Skipping.",
          e,
        );
      }
    }
