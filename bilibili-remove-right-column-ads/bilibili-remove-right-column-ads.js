...
(function() {
    ...
    function removeRightColumnAds() {
        ...
        let maxCol = 0;
        const allCards = feedContainer.querySelectorAll('[data-feed-card-row-col], [data-floor-card-row-col]');
        if (allCards.length === 0) return; // No cards loaded yet.

        allCards.forEach(card => {
            const rowCol = card.dataset.feedCardRowCol || card.dataset.floorCardRowCol;
            if (rowCol) {
        ...
        if (adsToRemove.length > 0) {
            console.log(`[Bilibili Ad Remover] Found and removed ${adsToRemove.length} ad cards in column ${maxCol}.`);
