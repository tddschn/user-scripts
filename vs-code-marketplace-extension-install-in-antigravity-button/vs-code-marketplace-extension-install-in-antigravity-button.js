...
(function () {
  ...
  function addButton() {
    ...
    const container = document.querySelector(
      ".installButtonContainer .ms-Fabric",
    );
    if (!container) return;

    // Find the existing install link to clone
    const existingLinkWrapper = container.querySelector(
      ".ux-oneclick-install-button-container",
    );
    const existingLink = existingLinkWrapper
      ? existingLinkWrapper.querySelector("a.install")
      : null;

    if (!existingLink || container.querySelector(".antigravity-install-button"))
      return;
    ...
    container.insertBefore(
      newLinkWrapper,
      container.querySelector(".installHelpInfo"),
    );
  }
