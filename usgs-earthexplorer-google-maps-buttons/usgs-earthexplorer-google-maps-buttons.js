...
(function () {
  ...
  const addGoogleMapsButton = (coordLiElement) => {
    ...
    const lat = decimalContainer
      .querySelector(".latitude")
      ?.textContent?.trim();
    const lon = decimalContainer
    ...
    if (lat && lon) {
      ...
      const operationsContainer = coordLiElement.querySelector(
        ".coordinateElementOperations",
      );
  ...
  const processAllCoordinates = () => {
    const coordEntries = document.querySelectorAll(
      'ul#coordEntryArea li[id^="coordinate_"]',
    );
