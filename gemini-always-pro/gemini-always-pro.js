...
(function() {
    ...
    const MODEL_PRIORITY = [
        { key: 'pro', patterns: [/^\s*pro\b/i, /\bpro\b/i, /gemini\s+advanced/i], name: 'Pro' },
        { key: 'thinking', patterns: [/^\s*thinking\b/i, /thinking-modus/i, /thinking\s+mode/i, /\bthinking\b/i], name: 'Thinking' },
        { key: 'fast', patterns: [/^\s*fast\b/i, /^\s*schnell\b/i, /\bfast\b/i, /\bschnell\b/i], name: 'Fast' }
    ];
    ...
    const ALL_MODEL_KEYWORDS = ['pro', 'thinking', 'fast', 'schnell', 'gemini advanced', 'thinking mode', 'thinking-modus'];
    ...
    function getModelRank(modelText) {
        ...
        if (/^thinking\b/.test(normalized) || /^thinking-modus\b/.test(normalized)) return 1;
        if (/^fast\b/.test(normalized) || /^schnell\b/.test(normalized)) return 2;

        for (let i = 0; i < MODEL_PRIORITY.length; i++) {
            if (MODEL_PRIORITY[i].patterns.some(pattern => pattern.test(normalized))) {
                return i;
            }
        }
    ...
    function getModePickerButton() {
        ...
        return buttons.find(button => {
            ...
            if (MODE_PICKER_ARIA_PATTERNS.some(pattern => ariaLabel.includes(pattern))) {
                return true;
            }

            const text = normalizeText(button.textContent);
            return button.getAttribute('aria-haspopup') === 'menu' && getModelRank(text) !== -1;
        }) || null;
    }
     ...
     */
    function processMenu(menuItems, modePickerButton, currentRank, currentModelText) {
        ...
        for (const item of menuItems) {
            ...
            const isDisabled = item.getAttribute('aria-disabled') === 'true' || (item instanceof HTMLButtonElement && item.disabled);
        ...
        console.log(`Gemini Always Pro v${SCRIPT_VERSION}: currentRank=${currentRank}, bestRank=${bestRank}, bestItemText=${bestItem ? (bestItem.textContent || '').trim() : 'none'}`);
        ...
        if (bestItem && bestRank < currentRank) {
            ...
            console.log(`Gemini Always Pro v${SCRIPT_VERSION}: Upgraded from ${currentModelText} to ${(bestItem.textContent || '').trim()}`);
        ...
        const hasRelevantChange = mutations.some(mutation => {
            ...
            for (const node of mutation.addedNodes) {
                if (node.nodeType === 1) { // ELEMENT_NODE
                    ...
                    if (text && ALL_MODEL_KEYWORDS.some(keyword => text.toLowerCase().includes(keyword))) {
                        return true;
                    }
                }
            }
