...
(function() {
    ...
    function parseWeight(title) {
        ...
        if (parenMatch) {
            return normalizeToGrams(parseFloat(parenMatch[1].replace(/,/g, '')), parenMatch[2]);
        }

        // Pattern 2: Look for direct metric ending: "500 g", "1 kg"
        const metricMatch = title.match(/([\d,.]+)\s*(g|mg|kg)(?!\w)/i);
        if (metricMatch) {
            return normalizeToGrams(parseFloat(metricMatch[1].replace(/,/g, '')), metricMatch[2]);
        }

        // Pattern 3: Look for Imperial units if metric is missing: "16 oz", "1 lb"
        const imperialMatch = title.match(/([\d,.]+)\s*(oz|lb)(?!\w)/i);
        if (imperialMatch) {
            return normalizeToGrams(parseFloat(imperialMatch[1].replace(/,/g, '')), imperialMatch[2]);
        }
    ...
    function processProduct(productEl) {
        ...
        const currencyMeta = productEl.querySelector('meta[itemprop="priceCurrency"]'); // Usually hidden elsewhere, defaulting to symbol detection
        let price = 0;
    ...
    function runScan() {
        ...
        const productCards = document.querySelectorAll('.product-cell-container, .product-inner');
    ...
    const targetNode = document.getElementById('FilteredProducts') || document.body;
